...
| `rmdir` | `<name>` | Remove a directory. Does not support non empty dirs. | `fs rmdir empty_dir`<br>`fs rmdir -r project_old`<br>`fs rmdir ../temp`<br>`fs rmdir /home/user/old_data`<br>`fs rmdir -r test_*` |
